...
`assets/template.html` bundles D2Coding (SIL OFL, `assets/D2Coding-OFL.txt`) alongside JetBrains Mono, inserted right after it in every rendered monospace font stack — which is nearly everywhere, since the Viewer chrome is monospace by design. D2Coding matches the JetBrains Mono cell width instead of pulling in a proportional UI font that would break alignment. Latin/digits still render in JetBrains Mono; any Hangul — in authored content or in the now-Korean Viewer chrome — falls through automatically to D2Coding. The one exception is the "editorial" preset's serif headings (`h1`, `.card h3`), which already fell back through a CJK serif stack (`Songti SC`/`STSong`) for `zh-CN`; that stack now also carries one weight of Pretendard (SIL OFL, `assets/Pretendard-OFL.txt`) for Hangul, ahead of the generic `serif` keyword. No per-diagram configuration needed for either the font or the translation catalog; all of it is baked into the template. Nothing to do here beyond writing Korean text as normal and setting `meta.locale: "ko"`.
