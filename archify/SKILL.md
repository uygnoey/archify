...
**Any well-formed locale tag works, for any language — not just the built-in ones.** `en`, `zh-CN`, and `es` ship as built-in Viewer-chrome catalogs (`renderers/shared/i18n.mjs`), so those three need nothing further. Every other locale is supplied as translation *data* you (the authoring agent) write into the document, not a renderer code change and not a fixed list anywhere in this repo. To localize a diagram in any other language, set `meta.locale` to its tag and write `meta.translations` yourself: an object mapping canonical message keys to translated strings whose `{placeholder}` tokens match the English source exactly. Translate from the English source itself, fresh for each diagram and in keeping with that diagram's own terminology — do not look up, copy, or adapt any checked-in translation file. Print the English source for every key with `node --input-type=module -e "import {catalogKeys,translateMessage} from './renderers/shared/i18n.mjs'; console.log(JSON.stringify(Object.fromEntries(catalogKeys().map((k) => [k, translateMessage('en', k)])), null, 2))"` from the skill root. `validate`/`render`/`deliver` report coverage to stderr (`archify: meta.translations for locale "…" covers N/519 renderer-owned messages…`); a key that is missing, unknown, or has mismatched placeholders falls back to its English string rather than breaking the render. `meta.locale` without a built-in catalog or `meta.translations` falls back entirely to English Viewer UI and `<html lang="en">`, with the same disclosure on stderr — never a half-translated page. The renderer itself never translates anything or calls a translation service; every string in `meta.translations` must already be present in the document when you call `validate`/`render`/`deliver`.
